...
contract WeatherOracleAuthorizationInsurance is Insurance, usingOraclize {
    ...
    function WeatherOracleAuthorizationInsurance() public {
    ...
    function __callback(bytes32 _myid, string _result) {
        ...
        updatedClaim(_result);
        if (keccak256(_result) == keccak256(expectedWeather)) {
            authoriseClaim(validIds[_myid]);
        }
    }
    ...
    function authoriseClaim(address _claimant) public {
        require(msg.sender == oraclize_cbAddress());
        if (insurances[_claimant].exists) {
            insurances[_claimant].withdrawable = true;
        }
    }

    function updateClaim(address _sender) payable {
